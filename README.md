...
| `SMTP_HOST` | **否** | SMTP服务器地址，如 `smtp.qq.com`。 | 无 |
| `SMTP_PORT` | **否** | SMTP服务器端口，通常为 `465` (SSL)。 | `465` |
| `SMTP_USER` | **否** | SMTP登录用户名，通常是邮箱地址。 | 无 |
| `SMTP_PASS` | **否** | SMTP登录密码或授权码。 | 无 |
| `SENDER_EMAIL` | **否** | 发件人邮箱地址，默认使用 SMTP_USER。 | 无 |
| `RECEIVER_EMAIL` | **否** | 收件人邮箱地址。 | 无 |
